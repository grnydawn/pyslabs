...
class LocalDomain():
    ...
    def __init__(self, nx_glob, nz_glob, data_spec, outfile, workdir):
        ...
        if self.is_master():
            ...
            self.slabs = pyslabs.parallel_open(outfile, mode="w")

            self.dens_writer = self.slabs.get_var("dens")
            self.umom_writer = self.slabs.get_var("umom")
            self.wmom_writer = self.slabs.get_var("wmom")
            self.rhot_writer = self.slabs.get_var("rhot")
